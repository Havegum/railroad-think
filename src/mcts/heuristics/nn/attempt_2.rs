...
mod builder {
    ...
    impl BuildOnDevice<DS, F> for ModelTwo {
        ...
        fn try_build_on_device(device: &DS) -> Result<Self::Built, <DS as HasErr>::Err> {
...
impl TensorCollection<F, DS> for ModelTwo<F, DS> {
    ...
    fn iter_tensors<V: ModuleVisitor<Self, F, DS>>(
        ...
        visitor.visit_fields(
            ...
            |(convolutional, heuristic, shared)| ModelTwo {
                convolutional,
                heuristic,
                shared,
            },
    ...
    )> for ModelTwo<F, DS>
{
    ...
    fn try_forward_mut(
        ...
        let conv_in = x.0;
        let heuristics_in = x.1;

        let x1: Tensor<Rank1<16>, F, DS, T> = self.convolutional.try_forward_mut(conv_in)?;
        let x2: Tensor<Rank1<16>, F, DS, T> = self.heuristic.try_forward_mut(heuristics_in)?;
        let x: Tensor<Rank1<32>, F, DS, T> = (x1, x2).try_concat_along(Axis::<0>)?;
